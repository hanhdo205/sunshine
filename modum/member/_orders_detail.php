...
if($infor_account_edit) {
	...
	$response_person = T_('Account deleted');
	$company = T_('Account deleted');
	$member_id = T_('Account deleted');
}
...
if(isset($_POST['submit_send']))
{
	...
	if(isset($_FILES['ship_file'])) {
		//printf("<pre>%s</pre>",print_r($_FILES['ship_file'],true));die();
			$uploaddirectory = "upload/shippables/";
			foreach($_FILES['ship_file']['name'] as $i => $name)
			{
				$number_check_arr = explode('_',$name);
				$number_check = $number_check_arr[0];
				if(@$_FILES["ship_file"]["name"][$i] != "" && $number_check == $order_number) {
					$isvalue = true;
					//printf("<pre>%s</pre>",print_r($_FILES['ship_file'],true));die();
					// now $name holds the original file name
					$tmp_name = $_FILES['ship_file']['tmp_name'][$i];
					$FileName = $_FILES["ship_file"]["name"][$i];
					...
					if($error === UPLOAD_ERR_OK )
					{
							...
							move_uploaded_file($tmp_name, $uploaddirectory.$utl->encode_filename($FileName));
							$file_class = '';
							
					}
					$_SESSION["order"]["stlfile"][$i] = $utl->encode_filename($FileName);
					...
					if($_SESSION["order"]["stlfile"][$i] == "") {
						...
						$_SESSION["order"]["fileerror"][$i] = T_("Prefix number is not equal");
					}
					unset($_SESSION["order"]["stlfile"][$i]);
				}
			}

	}
	$emailable = false;
	if(isset($_POST["downloadable"]) && $_POST["downloadable"] == 1) {
	...
	//$downloadable = (isset($_POST["downloadable"]) && $_POST["downloadable"] == 1) ? $_POST["downloadable"] : 0;
	$payment_status = 0;
	
	if($paid && $paid >= $total) {
		$payment_status = 1;
	} elseif($paid && $paid < $total) {
		$payment_status = 3;
	}
	
	switch(true) {
		case ($rowgetInfo["roles_id"]==4) : // admin role only
			if(isset($_POST['manager_id']) && $_POST['manager_id']!='') {
				if($status==0) $status = 1;
				elseif($status==3 && $downloadable == 0 && $isvalue) $status = 4;
				elseif($status==3 && $downloadable == 1 && $isvalue) $status = 5;
				$manager_class = '';
				...
				if($isvalue) {
					...
					$array_orders['shipping_date'] = $shipping_date;
					$array_orders["checker_id"]=0;
					$array_orders["checker"]='';
				}
			}
		...
		case ($rowgetInfo["roles_id"]==6) : // from manager role
			if(isset($_POST['operator_id']) && $_POST['operator_id']!='') {
				...
				$array_orders = array("dateupdated"=>$dateupdated,"status"=>$status,"manager"=>$manager_name,"manager_id"=>$manager_id,"operator"=>$operator_name,"operator_id"=>$operator_id,"downloadable"=>$downloadable);
				if($isvalue) {
					$array_orders['read_list'] = "";
					$array_orders['shipping_date'] = $shipping_date;
					$array_orders["checker_id"]=0;
		...
		case ($rowgetInfo["roles_id"]==7 && $isvalue) : //from operator role and shippable date uploaded
			...
			elseif($status==3 && $downloadable == 1) $status = 5;
				$array_orders = array("read_list"=>"","dateupdated"=>$dateupdated,"status"=>$status,"shipping_date"=>$shipping_date,"checker_id"=>0,"checker"=>'');
			break;
	}
	...
	if($orders_arr > 0) { // if updated OK
		...
		foreach($_SESSION["order"]["detail_id"] as $i=>$v) {
			...
			$remarks = ($_SESSION["order"]["remarks"][$i] != '') ? $_SESSION["order"]["remarks"][$i] : '';
			$process_file = $_SESSION["order"]["process_file"][$i];
			$array_detail = array();
			$stlfile = (isset($_SESSION["order"]["stlfile"][$i]) && $_SESSION["order"]["stlfile"][$i] != '') ? $_SESSION["order"]["stlfile"][$i] : $_SESSION["order"]["ship_file_text"][$i];
			
			if($rowgetInfo["roles_id"]<8) {
				if($rowgetInfo["roles_id"]==4) {
					//$array_detail["shade"]=$shade;
					$array_detail["position"]=$position;
					$array_detail["detail_quantity"]=$detail_quantity;
					$array_detail["desired_date"]=$desired_date;
					$array_detail["remarks"]=$remarks;
				}
				if(isset($_SESSION["order"]["stlfile"][$i]) && $_SESSION["order"]["stlfile"][$i] != '') {
					$shipping_password = $utl->generateRandomString(10);
					//die($stlfile);
					//$array_detail = array("ship_file"=>$stlfile,"shipping_password"=>$shipping_password);
					$array_detail["ship_file"]=$stlfile;
					$array_detail["detail_updated"]=$dateupdated;
					//$msg_detail .= sprintf(T_('Patient: %s - Password to download: %s <br>'),$patient_name,$shipping_password);
				}
				$msg_detail .= '患者様名: ' . $patient_name . '様';
				switch($customer_language) {
					case 'ja_JP' :
						//$customer_msg_detail .= '患者様名: '.$patient_name.' ダウンロード用パスワード: '.$shipping_password.' <br>';
						$customer_msg_detail .= '患者様名: '.$patient_name.'様<br>';
						break;
					case 'vi_VN' :
	...
	if($emailable) {
		...
		$subject = "[VietQuocLab]データの準備が完了しました";
		$header = 'データの準備が完了しました';
		$admin_text = '下記のリンクをクリックして、ログインの上、データをダウンロードしてください';
		...
		switch($customer_language) {
			case 'ja_JP' :
				...
				$mail_footer_text = '弊社のURLは: ';
				$customer_text = '下記のリンクをクリックして、ログインの上、データをダウンロードしてください';
				break;
			case 'vi_VN' :
				$customer_subject = '[VietQuocLab]Dữ liệu　tải về';
				...
				break;
			default :
				$customer_subject = '[VietQuocLab]Data is ready for download';
		...
		$admin_message = Template::get_contents("modum/mail_template/shipable_upload.tpl", array('logo' => HOST.'images/logo.jpg', 'subject' => $header,'msg_detail' => $msg_detail,'order_detail' => $order_detail,'link' => $admin_order_link,'customer_text'=>$admin_text,'mail_footer_text'=>$mail_footer,'url' => HOST));
...
<main class="main">
	...
	<ol class="breadcrumb">
	  ...
	  <li class="breadcrumb-item">
		<a href="orders.aspx"><?php echo T_('Orders');?></a>
	  </li>
		...
		<div class="row">
		  <div class="col-md-12">
			...
			<div class="card">
			  ...
			  <div class="card-body">
				<?php //printf("<pre>%s</pre>",print_r($array_detail,true));//printf("<pre>%s</pre>",print_r($_SESSION["order"],true));  
				//echo $v;
				?>
						...
						<fieldset class="col-3 form-group">
							<div class="mb-2">
							  ...
							  <div class="input-group">
								<input class="form-control" id="order_number" type="text" name="order_number" placeholder="Order Number" value="<?php echo $order_number;?>"/>
							  </div>
						  	</div>
							<div>
						...
						<fieldset class="col-3 form-group">
							...
							<div class="<?php echo $operator_class;?>"> 
								...
								<?php if($rowgetInfo["roles_id"]==7) { ?>
									<select class="form-control select2-single" id="select2-3">
									  <option value="">---</option>
									  <?php
							...
							<?php if($rowgetInfo["roles_id"]==6 && $status > 3) { ?>
								<div class="form-group row mt-4">
								  <label class="col-md-8"><?php echo T_('Mark as paid');?></label>
									  <div class="col-md-4">
									  <label class="switch switch-label switch-outline-primary">
										<input class="switch-input markaspaid" type="checkbox" value="1" name="downloadable" <?php echo ($utl->checked(array($downloadable),1)) ? 'checked' : '';?>>
			  ...
			  <div id="bookingdetailAccordion" data-children=".item">
				  ...
				  <div class="item">
									...
									<div class="form-group row info_group mt-3">
											...
											<div class="form-group row">
												...
												<div class="col-md-9">
													<div class="input-group">
														...
														<input id="date" class="form-control date desired_date" type="text" name="desired_date[]" value="<?php echo $order_detail['desired_date'];?>"/>
											...
											<div class="proce-data">
												...
												<div class="proce-data-content">
													<p class="proce-data-content-txt">
														<?php echo ($operator_id > 0) ? urldecode($order_detail['process_file']) : T_('Please select the operator person');?>
													</p>
										...
										<div class="col-sm-3 custom-proce-data  mb-3 ship_file<?php echo $file_class;?>">
											<div class="proce-data">
												...
												<div class="proce-data-content shippable_file_content">
													...
													<?php } else { ?>
														<p class="proce-data-content-txt ship_file_uploaded_<?php echo $key;?>">
															...
															if($rowgetInfo["roles_id"]<7) {
																$text_tmp = ($status >= 3) ? T_('No file uploaded') : T_('Please download manufacture file');
																//if($status >= 3) {
																	$ship_downloaded = '';
																	if($checker_id) $ship_downloaded = '✓';
																	echo ($order_detail['ship_file']) ? '<a href="javascript:void(0)" class="btn btn-primary btn-custom download_file download_file_shippable" data-id="'. $order_id .'" data-user="'.$_SESSION['member_id'].'" data-ship="'.$order_detail['ship_file'].'">'. T_('Check it').'</a> ' . $ship_downloaded : $text_tmp;
																//}
															} else {
													...
													<?php } ?>
														<p class="btn-text text-center">
															...
															<input type="hidden" name="ship_file_text[]" value="<?php echo $order_detail['ship_file'];?>">
															<?php } else { 
																echo '';
															} ?>
														</p>
					...
					<script type="text/javascript">
						$(document).ready(function () {
							...
							fileSelectEle.onchange = function (e)
							{
								var file = e.target.files[0];
								//console.log(file.name);
								var file_name = file.name;
								var prefix_arr = file_name.split('_');
								var prefix = prefix_arr[0];
								$('.error_file_name').remove();
								...
								if(fileSelectEle.value.length == 0) {
									...
									if($('#order_number').val() !== prefix) {
										//fileSelectEle.val('');
										$('.shippable_file_content').prepend('<div class="col error_file_name"><div class="alert alert-danger alert-dismissible fade show file_field_error_<?php echo $key;?>" role="alert"><?php echo T_("Prefix number is not equal");?><button class="close" type="button" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">×</span></button></div></div>');
									} else {
							...
							<?php if(isset($_POST['submit_send'])) { 
								if(!($_SESSION['count'])){
									$_SESSION['count'] = 2;
								}else{
			   ...
			   <div class="card-footer">
				  ...
				  <a href="javascript:void(0);" onclick="goBack(-1)" id="goback" class="btn btn-link btn-lg active" role="button" aria-pressed="true"><i class="fa fa-angle-double-left" aria-hidden="true"></i> <?php echo T_('Go back to Order List');?></a>
					<?php if($rowgetInfo["roles_id"]!=8) { ?>
						<button id="save_changes" class="btn btn-primary btn-custom" type="submit" name="submit_send">
						<?php echo T_('Save Changes');?></button>
					<?php } ?>
				</div>
			</form>
			</div>

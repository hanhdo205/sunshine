...
if(strstr($image,$docRoot)!="")
{
   $absoluteimg=$image;
   $absoluteimg	= preg_replace('/^\//', '', $absoluteimg);
}
else
{
  ...
  $absoluteimg=$docRoot . $image;
}
// Get the size and MIME type of the requested image
$size	= GetImageSize($absoluteimg);
$mime	= $size['mime'];
...
{
	header('HTTP/1.1 400 Bad Request');
	echo 'Error: requested file is not an accepted type: ' . $absoluteimg;
	exit();
}
...
{
	$data	= file_get_contents($absoluteimg);
	$lastModifiedString	= gmdate('D, d M Y H:i:s', filemtime($absoluteimg)) . ' GMT';
	$etag				= md5($data);
...
{
	$imageModified	= filemtime($absoluteimg);
...
$src	= $creationFunction($absoluteimg);
